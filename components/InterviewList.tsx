...
export const InterviewList: React.FC<InterviewListProps> = ({ onNew, onEdit }) => {
  ...
  const filteredRecords = records.filter(r =>
    r.basicInfo.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    r.basicInfo.position.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (r.basicInfo.store && r.basicInfo.store.toLowerCase().includes(searchTerm.toLowerCase())) ||
    Object.values(r.answers).some((answer: string) => answer.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {portalTarget && createPortal(
        <Button onClick={onNew} variant="purple" className="shadow-sm">
      ...
      <div className="relative mb-6">
        ...
        <input
          type="text"
          className="block w-full pl-10 pr-3 py-3 bg-white border border-elleo-purple rounded-xl leading-5 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-elleo-purple/20 sm:text-sm shadow-sm transition-shadow"
          placeholder="지원자명, 포지션, 매장, 내용 검색..."
          value={searchTerm}
          ...
          {filteredRecords.map(record => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="text-lg font-bold text-slate-800 group-hover:text-elleo-dark transition-colors">{record.basicInfo.name}</h3>
                  <span className="inline-block px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-600 mt-1">
                    {record.basicInfo.position}
                  </span>
                </div>
                <button
              ...
              <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                <span className="text-xs text-slate-400">
                  메모 {Object.keys(record.answers).filter(k => record.answers[k]).length}개
                </span>
                {record.aiSummary && (
